        ...
        <div class="table">
            <el-table :data="userList" border style="width: 100%">
                ...
                <el-table-column prop="status" label="状态" width="160">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.status  === '0' ? 'success' : 'info' " :color="'#ffffff'" :hit="false" :size="'mini'">{{scope.row.status === '0' ? '启用' : '禁用'}}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="160">
                    <template slot-scope="scope">
                        <el-button size="mini" type="text" v-if="scope.row.status === '0'" @click="enable(scope.$index, scope.row)">停用</el-button>
                        <el-button size="mini" type="text" v-else-if="scope.row.status === '1'" @click="enable(scope.$index, scope.row)">启用</el-button>
                        ...
                        <el-popover :ref="scope.row.id" placement="top" width="150" trigger="click">
                            ...
                            <div style="text-align: right; margin: 0">
                                ...
                                <el-button type="primary" size="mini" @click="resetPwd(scope.row)">确定</el-button>
                            </div>
...
export default {
    ...
    methods: {
        ...
        resetPwd(user) {
            let api = `/user/reset/${user.id}`
            this.$fetch.post(api).then(res => {
                if (res.code === 10000) {
                    this.$notify({
                        title: "提示",
                        message: `用户【 ${user.userName} 】密码已重置为: 888888`,
                        type: "success"
                    });
                } else {
                  this.$message({
