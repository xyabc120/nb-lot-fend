...
<div class="pageinfo">
        ...
        <div class="card mt20">
            <el-row :gutter="10">
                        ...
                        <el-col :span="8">
                            <el-row style="line-height:27px;">
                                ...
                                <el-col :span="16" class="value" title="点击选择">
                                    <el-button v-if="!(stepVal == workFlow.length)" size="mini" type="text" @click="dialogFormVisible=true,dispatchUser = workOrder.dispatchUserLoginName">{{workOrder.dispatchUser}}</el-button>
                ...
                <el-col :span="10" class="value">
                    ...
                    <div v-if="workOrder.amapLongitude && workOrder.amapLatitude" class="amap-container mb20">
                        <el-amap class="amap-box" :vid="'amap'+new Date()" :zoom="11" :center="[workOrder.amapLongitude, workOrder.amapLatitude]">
                            <el-amap-marker :position="[workOrder.amapLongitude, workOrder.amapLatitude]" :content="dom_jg" :animation="'AMAP_ANIMATION_DROP'" :offset="[-16,-14]"></el-amap-marker>
                        </el-amap>
                    </div>
                    <el-row>
    ...
    <div class="pageinfo-content mt10">
        <div class="card">
            ...
            <div class="block">
                <el-timeline>
                    <el-timeline-item v-for="(record,index) in recordList" :key="record.id" :icon="index ? '' : 'el-icon-more' " :type="index ? '' : 'primary' " :timestamp="record.submitUser+'  '+record.createDate" placement="top">
                        <el-card>
                            <p>{{record.remark}}</p>
                        </el-card>
                        <div v-if="record.files.length>0" class="img-list mt10">
                            <el-upload list-type="picture-card" action="#" :on-preview="handlePictureCardPreview" :disabled="true" :limit='3' :file-list="record.files">
                            </el-upload>
                        </div>
                    </el-timeline-item>
                </el-timeline>
            ...
            <div v-if="!(stepVal == workFlow.length)" class="comment mb20">
                <el-form ref="commentForm" :rules="rules" :model="comment" label-width="60px" size="mini">
                    ...
                    <el-form-item>
                        <el-button type="primary" @click="SaveComment('commentForm')">保存备注</el-button>
                    </el-form-item>
                </el-form>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        uploadOk(res, file, fileList) {
            ...
            this.comment.fileList.push(temp)
        },
        removeOk(file, fileList) {
            let i = this.comment.fileList.findIndex((value, index, arr) => {
                return value === file
            });
        ...
        uploadOk2(res, file, fileList) {
           let temp = {
            ...
            this.submitData.files.push(temp)
        },
        removeOk2(file, fileList) {
          let i = this.submitData.files.findIndex((value, index, arr) => {
                return value === file
            });
            this.submitData.files.splice(i,1)
        },
...
.block .img-list .el-upload--picture-card {
    display: none;
}

.block .img-list .el-upload-list li .el-upload-list__item-status-label {
    display: none;
}
</style>
